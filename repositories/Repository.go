...
type Repository interface {
	...
	List(db *sqlx.DB, record any, query *qapi.Query, lang ...string) (int, error)

	// Read combines previous Read and ReadSmartSelect
	// If record is of type *E, performs regular read, otherwise does smart select
	Read(db *sqlx.DB, record any, id any, preloads ...string) error

	Create(db *sqlx.DB, record any) error
	...
	Update(db *sqlx.DB, record any, fieldParams ...map[string]any) error
	...
	Delete(db *sqlx.DB, record any, ids ...any) error
}
